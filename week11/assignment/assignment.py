...
def security(passenger_list, lock):
    ...
    """
    while len(passenger_list) < TOTAL_PASSENGERS:
        lock.acquire()
...
def cruise_director(passenger_list, lock_directors, lock_security, directors_count):
    ...
    """
    while ALL_DONE not in passenger_list:
        # if not lock_security.locked():
        director_waiting()
        # if not security.lock: 
       
        lock_directors.acquire()
        if director == 0:
            print(STARTING_READING_MESSAGE)
            directors_count += 1
        if director == DIRECTOR_PERSONAL - 1:
            print(STOPPING_READING_MESSAGE)            
            print(f'Director reading: list size is {len(passenger_list)}')
        director_processing()
        lock_directors.release()
