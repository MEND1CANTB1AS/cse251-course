...
def merge_sort_process(arr):
    ...
    if len(arr) > 1:
        ...
        # Sorting the first half
        l_process = mp.Process(target=merge_sort_process, args=(L,))
 
        # Sorting the second half
        r_process = mp.Process(target=merge_sort_process, args=(R,))
        l_process.start()
        r_process.start()
        l_process.join()
        r_process.join()
...
def main():
    ...
    for merge_function, desc in merges:
        ...
        arr = [random.randint(1, 10000000) for _ in range(15)]
        print(f'\n{desc:-^90}')
