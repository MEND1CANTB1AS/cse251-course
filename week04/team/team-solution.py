...
def fix_name(name):
    return name.replace('Luke', 'Luc')

def retrieve_thread(data_queue, data_sem, log):
    ...
    while True:
        ...
        if response.status_code == 200:
            data = response.json()
            log.write(fix_name(data['name']))
        else:
            log.write(f'ERROR with {value}')


def file_reader(filename, data_queue, data_sem, log):
...
def main():
    ...
    # create the threads
    reader = threading.Thread(target=file_reader, args=('data.txt', data_queue, data_sem, log))
    workers = []
    for _ in range(RETRIEVE_THREADS):
        workers.append(threading.Thread(target=retrieve_thread, args=(data_queue, data_sem, log)))
